...
load rho.mat
rho = rho(:,2,dep_bnds,:);
E = squeeze(cumsum((rho(:,:,:,3:end)-rho(:,:,:,1:end-2)),4));
...
rho_trend = NaN(size(E,2),size(E,1));
rho_sig = rho_trend;

for j = 1:size(E,1)
    for k = 1:size(E,2)
        [rho_trend(k,j) ~, rho_sig(k,j)] = trend(squeeze(E(j,k,:)),99);
    end
end

rho_trend(rho_sig == 0) = NaN;
...
load redblue
colormap(redblue)
[a b] = contourf(slon,depth2',12*100*rho_trend);
...
[q, h] = contour(slon,depth2',avg_rho',[0 0 0],'k','LineWidth',2);

[qq, hh] = contour(slon,depth2',avg_rho',(.5:.5:2)*10^(-7),'k');
[qqq, hhh] = contour(slon,depth2',avg_rho',(.5:.5:2)*-10^(-7),':k');
