...
% sum of integration
sum_center_dif = cumsum(center_dif,3);


 %% calculating trend via forward differencing - 2nd method
 
 % forward difference 
for_dif = (u_eq(:,:,2:end)-u_eq(:,:,1:(end-1)));
% average slope
avg_dif_2 =  squeeze(sum(for_dif,3)/(nt-2));
% sum of integration
sum_for_dif = cumsum(for_dif,3);
...
euc_sig = euc_trend;
forward_trend = euc_trend;
center_trend = euc_trend;

for j = 1: length(I_lon)
    for k = 1:length(depth)
    
    [forward_trend(k,j),euc_plusminus(k,j),euc_sig(k,j)]=trend_stat((squeeze(squeeze(sum_for_dif(j,k,:)))),99);   
    [center_trend(k,j),euc_plusminus(k,j),euc_sig(k,j)]=trend_stat((squeeze(squeeze(sum_center_dif(j,k,:)))),99);
...
subplot(3,1,1)
[a b] = contourf(slon,depth,12*100*center_trend,300);
set(b,'EdgeColor','none')
...
set(gca,'YDir','reverse')
caxis([-.3 0.3])
title('Trend by Regressing Cumulative Sum of Central Differences of u (m sec^{-1} century^{-1})')
colorbar
...
% set(gca,'YDir','reverse')
% caxis([-.3 0.3])
% title('Trend by Average Central Difference of u (m sec^{-1} century^{-1})')
% colorbar

subplot(3,1,2)
[a b] = contourf(slon,depth,12*100*forward_trend,300);
set(b,'EdgeColor','none')
...
set(gca,'YDir','reverse')
caxis([-.3 0.3])
title('Trend by Regressing Cumulative Sum of Forward Differences of u(m sec^{-1} century^{-1})')
colorbar
...
% set(gca,'YDir','reverse')
% caxis([-.3 0.3])
% title('Trend by Average Forward Difference of u (m sec^{-1} century^{-1})')
% colorbar
