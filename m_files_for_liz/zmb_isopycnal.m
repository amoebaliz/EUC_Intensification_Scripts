function [lon,lat,depth,temp_iso, u,dudt,ududx,vdudy,wdudz,zpgf,corf,frich,fricv] = zmb_isopycnal(lon,lat,depth,u,v,w,temp,salt,ssh)
...
g=-9.81;
omega=7.292*10^(-5);
Ah=1.5*10^3; % Wallcraft et al. (2005, GRL)
...
temp_iso=NaN(length(lon),length(lat),length(isopyc_layer_bnds)-1);
    ...
    for y=1:length(lat)
        for layer=1:length(isopyc_layer_bnds)-1
            ...
            if isfinite(zrange)
                ...
                temp_iso(x,y,layer)=nanmean(temp(x,y,depth>=zrange(1)&depth<=zrange(length(zrange))),3);
            end
        end
    end
