...
% NOTE: assuming lat, lon, depth, time) are the same for u, v, and w 
 ...
 u_clim = zeros(12, length(lat), length(lon), length(dept));
 v_clim = zeros(12, length(lat), length(lon), length(dept));
 w_clim = zeros(12, length(lat), length(lon), length(dept));
 
% Initialize u/v/w_anom variables
  
 u_anom = zeros(length(time), length(lat), length(lon), length(dept));
 v_anom = zeros(length(time), length(lat), length(lon), length(dept));
 w_anom = zeros(length(time), length(lat), length(lon), length(dept));
...
% lon x lat planes: specify depth
planeimg1 = squeeze(u_clim_trend(:,:,i_depth(1),month));        % upper plane
planeimg2 = squeeze(u_clim_trend(:,:,i_depth(end),month));      % lower plane

% lon x depth planes: specify latitude
planeimg3 = squeeze(u_clim_trend(:,i_lat(1),:,month));          % southern plane
planeimg4 = squeeze(u_clim_trend(:,i_lat(end),:,month));        % northern plane

% lat x depth planes: specify longitude
planeimg5 = squeeze(u_clim_trend(i_lon(1),:,:,month));          % western plane
planeimg6 = squeeze(u_clim_trend(i_lon(end),:,:,month));        % eastern plane


% TREND; data are in dimensions: <lon x lat x depth x month>
% lon x lat planes: specify depth
planeimg7 = squeeze(u_clim_trend(:,:,i_depth(1),month));        % upper plane
planeimg8 = squeeze(u_clim_trend(:,:,i_depth(end),month));      % lower plane

% lon x depth planes: specify latitude
planeimg9 = squeeze(u_clim_trend(:,i_lat(1),:,month));          % southern plane
planeimg10 = squeeze(u_clim_trend(:,i_lat(end),:,month));        % northern plane

% lat x depth planes: specify longitude
planeimg11 = squeeze(u_clim_trend(i_lon(1),:,:,month));          % western plane
planeimg12 = squeeze(u_clim_trend(i_lon(end),:,:,month));        % eastern plane
...
[C h] = contourf(slon,depth1,planeimg1,100,'LineWidth',2);
...
% Equator panel
h12 = surf([min_x max_x],-1*[min(depth1) max(depth1)],repmat(-1*imgzposition3-.1, [2 2]),...
...
% upper panel
h12 = surf([min_x max_x],-1*[min(depth1) max(depth1)],repmat(-1*imgzposition4-.1, [2 2]),...
       NaN(size(planeimg7)),'facecolor','texture','FaceAlpha',1);
   
[C h] = contourf(slon,lat,planeimg7,100,'LineWidth',2);
set(h, 'EdgeColor','none')
...
% lower panel
h12 = surf([min_x max_x],-1*[min(depth1) max(depth1)],repmat(imgzposition4-.1, [2 2]),...
...
% West Plane
h12 = surf([min(depth1) max(depth1)],-1*[min_y max_y],repmat(imgzposition5-.1, [2 2]),...
...
% East Plane
h12 = surf([min(depth1) max(depth1)],-1*[min_y max_y],repmat(imgzposition6-.1, [2 2]),...
      .75*ones(size(planeimg3)),'facecolor','texture','FaceAlpha',1);
